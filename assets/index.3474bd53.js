...
`;function G(e){let r;return q.combine(((e,o)=>{void 0!==r?(o(e()-r),r=e()):r=e()}),[e])}const K=q.merge(q(document.hasFocus()),q.merge(j(window,"focus"),j(window,"blur")).map(f("type")).map(m("focus"))),H=new U;H.domElement.parentElement.style.zIndex=Number.MAX_SAFE_INTEGER.toString();const X=function(e){const r=q.combine(((e,r)=>{if(!e())return function(){const e=q(),r=q(),o=q.merge(q.of(0),q.immediate(q.combine((r=>(e(requestAnimationFrame(r)),r())),[r])));return q.on((()=>cancelAnimationFrame(e())),o.end),o}().pipe(G);r()&&r().end(!0)}),[e]),o=(t=r,q.combine(((e,r)=>{e().map(r)}),[t]));var t;return q.scan(u,0,o)}(K.map(v).map(((...e)=>c(i(console.log,e)))("pause"))),J=G(X).map((e=>e/1e3)),Q=(V=document,q.scan(((e,r)=>(e[r.key]="keydown"===r.type,e)),{},q.merge(j(V,"keydown"),j(V,"keyup"))));var V;const Z=e=>e<-1?e+2:e>1?e-2:e,ee=({frag:e})=>`\n  for (float x = -2.; x <= 2.; x += 2.) {\n    for (float y = -2.; y <= 2.; y += 2.) {\n      vec2 pScreenWrap = vec2(x, y);\n      ${e}\n    }\n  }\n`,re=H.addFolder("asteroid"),oe=(e=1)=>(2*Math.random()-1)*e,te=(e=1)=>g(w(),$(w(),h(oe(),oe())),e),ne=y(0,10),ae=q.scan(((e,r)=>{return e.map((o=r,e=>a(a({},e),{p:b(w(),e.p,g(w(),e.v,o)).map(Z)})));var o}),ne.map((()=>({p:h(oe(),oe()),v:te(.25*Math.random()),size:Math.random()}))),J),se=z`
  ${{uniforms:ne.reduce(((e,r,o)=>{for(const t of Object.keys(ae()[o]))e[`asteroids[${o}].${t}`]=()=>ae()[o][t];return e}),{})}}
  
  struct Asteroid { vec2 p; vec2 v; float size; };
  uniform Asteroid asteroids[${10}];

  float sdAsteroid(vec2 p, Asteroid a) {
    float d = sdCircle(p - a.p, a.size * ${N(re.auto(.1,"sizeScale",.1,.5))} +
      ${N(re.auto(.2,"sizeBase",.01,.2))});
    return d;
  }
  ...
  float sdAsteroids(vec2 p) {
    ...
    for (int i = 0; i <= ${10}; i+= 1) {
      d = opSmoothUnion(d, sdAsteroid(p, asteroids[i]), ${N(re.auto(.1,"u_asteroidK",0,.25))});
    }
    return d;
  }
  ...
  vec4 asteroidsColor(vec2 p) {
    ...
    d = -d;
    d = d * ${N(re.auto(30,"bandEffectScaleUp",1,50))};
    d = floor(d) + 1.;
    d = d / ${N(re.auto(3,"bandEffectScaleDown",1,10))};
    d = normalized(d);

    float stepD = step(sdSpaceAsteroids(p), 0.);
    float alpha = mix(d, stepD, ${N(re.auto(0,"bandOrStep",0,1))});
    
    return vec4(${N(re.auto("#d79552","asteroidColor"))}, alpha);
  }
`,ce=e=>r=>(...o)=>r(e(),...o),ie=ce(w),le=ie(b),pe=ie(g),de=ie($),ue=ie(x),fe=A,me=ce(S)(I);F(Array);const ve=H.addFolder("player"),ye=Q.map((({a:e,d:r,w:o,s:t})=>[r?1:e?-1:0,o?1:0])),be=q.scan(((e,r)=>{const[o]=ye();return e-o*ve.auto(1.5,"rotate",.1,2)()*Math.PI*r}),0,q.combine((e=>0!==ye()[0]?e():void 0),[J])),ge=[0,-1],we=be.map(C(me,(e=>ue(ge,e)))),he=J.map((e=>we().map((r=>r*(ye()[1]*ve.auto(.05,"acceleration",.025,.1)()*e))))),$e=ve.auto(.02,"maxSpeed",.005,.05).map((e=>r=>fe(r)>e?pe(de(r),e):r)),xe=q.scan(C(le,(e=>$e()(e))),[0,0],he);var Ae;const Ie=z`
  ...
  float sdPlayer(vec2 p) {
    vec2 pos = ${N(q.scan(C(le,(Ae=Z,e=>e.map(Ae))),[0,0],xe))};
    vec2 center = vec2(0, 0.06);
    vec2 player = p - center - pos;
    player = rotate2d(${N(be)}) * player;
    player += center + pos;
